...
class Wine {
    ...
    plant() {
        ...
        if (!!!packageConfig.wine.grapeseedsconfig) return Wine.sip('This package has a wine.json but no grape-seeds.json file.');
        if (!(fs.existsSync(`./node_modules/${exportingPackageName}/${packageConfig.wine.grapeseedsconfig}`) || fs.existsSync(`./node_modules/${exportingPackageName}/grape-seeds.json`))) return Wine.sip('This package is not a wine package.');

        let grapeseedsConfig;
        try {
            try {
                grapeseedsConfig = require(`./node_modules/${exportingPackageName}/${packageConfig.wine.grapeseedsconfig}`);
            } catch (e) {
